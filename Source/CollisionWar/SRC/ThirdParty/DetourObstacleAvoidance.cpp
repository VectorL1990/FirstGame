...
dtObstacleAvoidanceDebugDataTP* dtAllocObstacleAvoidanceDebugData()
{
	void* mem = dtAlloc(sizeof(dtObstacleAvoidanceDebugDataTP), DT_ALLOC_PERM);
	if (!mem) return 0;
	return new(mem) dtObstacleAvoidanceDebugDataTP;
}

void dtFreeObstacleAvoidanceDebugDataTP(dtObstacleAvoidanceDebugDataTP* ptr)
{
	if (!ptr) return;
	ptr->~dtObstacleAvoidanceDebugDataTP();
	dtFreeTP(ptr);
}


dtObstacleAvoidanceDebugDataTP::dtObstacleAvoidanceDebugDataTP() :
	m_nsamples(0),
	m_maxSamples(0),
	m_vel(0),
	m_ssize(0),
...
bool dtObstacleAvoidanceDebugDataTP::init(const int maxSamples)
{
	dtAssert(maxSamples);
...
void dtObstacleAvoidanceDebugDataTP::reset()
{
	m_nsamples = 0;
}

void dtObstacleAvoidanceDebugDataTP::addSample(const float* vel, const float ssize, const float pen,
...
void dtObstacleAvoidanceDebugDataTP::normalizeSamples()
{
...
dtObstacleAvoidanceQueryTP* dtAllocObstacleAvoidanceQuery()
{
	void* mem = dtAlloc(sizeof(dtObstacleAvoidanceQueryTP), DT_ALLOC_PERM);
	if (!mem) return 0;
	return new(mem) dtObstacleAvoidanceQueryTP;
}

void dtFreeObstacleAvoidanceQueryTP(dtObstacleAvoidanceQueryTP* ptr)
{
	if (!ptr) return;
	ptr->~dtObstacleAvoidanceQueryTP();
	dtFreeTP(ptr);
}


dtObstacleAvoidanceQueryTP::dtObstacleAvoidanceQueryTP() :
	m_invHorizTime(0),
	m_vmax(0),
	m_invVmax(0),
	m_maxCircles(0),
...
bool dtObstacleAvoidanceQueryTP::init(const int maxCircles, const int maxSegments)
{
...
void dtObstacleAvoidanceQueryTP::reset()
{
	m_ncircles = 0;
	m_nsegments = 0;
}

void dtObstacleAvoidanceQueryTP::addCircle(const float* pos, const float rad,
...
void dtObstacleAvoidanceQueryTP::addSegment(const float* p, const float* q)
{
...
void dtObstacleAvoidanceQueryTP::prepare(const float* pos, const float* dvel)
{
	// Prepare obstacles
	...
	for (int i = 0; i < m_nsegments; ++i)
	{
		...
		const float r = 0.01f;
		float t;
		seg->touch = dtDistancePtSegSqr2DTP(pos, seg->p, seg->q, t) < dtSqr(r);
	}	
}
 ...
 */
float dtObstacleAvoidanceQueryTP::processSample(const float* vcand, const float cs,
											  const float* pos, const float rad,
											  const float* vel, const float* dvel,
											  const float minPenalty,
											  dtObstacleAvoidanceDebugDataTP* debug)
{
...
int dtObstacleAvoidanceQueryTP::sampleVelocityGrid(const float* pos, const float rad, const float vmax,
												 const float* vel, const float* dvel, float* nvel,
												 const dtObstacleAvoidanceParamsTP* params,
												 dtObstacleAvoidanceDebugDataTP* debug)
{
	prepare(pos, dvel);
	
	memcpy(&m_params, params, sizeof(dtObstacleAvoidanceParamsTP));
...
int dtObstacleAvoidanceQueryTP::sampleVelocityAdaptive(const float* pos, const float rad, const float vmax,
													 const float* vel, const float* dvel, float* nvel,
													 const dtObstacleAvoidanceParamsTP* params,
													 dtObstacleAvoidanceDebugDataTP* debug)
{
