...
dtProximityGridTP* dtAllocProximityGrid()
{
	void* mem = dtAlloc(sizeof(dtProximityGridTP), DT_ALLOC_PERM);
	if (!mem) return 0;
	return new(mem) dtProximityGridTP;
}

void dtFreeProximityGridTP(dtProximityGridTP* ptr)
{
	if (!ptr) return;
	ptr->~dtProximityGridTP();
	dtFreeTP(ptr);
}
...
dtProximityGridTP::dtProximityGridTP() :
	m_cellSize(0),
	m_invCellSize(0),
	m_pool(0),
	m_poolHead(0),
...
bool dtProximityGridTP::init(const int poolSize, const float cellSize)
{
	dtAssert(poolSize > 0);
...
void dtProximityGridTP::addItem(const unsigned short id,
...
int dtProximityGridTP::queryItems(const float minx, const float miny,
								const float maxx, const float maxy,
...
int dtProximityGridTP::getItemCountAt(const int x, const int y) const
{
	int n = 0;
