...
dtLocalBoundaryTP::dtLocalBoundaryTP() :
	m_nsegs(0),
	m_npolys(0)
{
...
void dtLocalBoundaryTP::addSegment(const float dist, const float* s)
{
...
void dtLocalBoundaryTP::update(dtPolyRef ref, const float* pos, const float collisionQueryRange,
							 dtNavMeshQueryTP* navquery, const dtQueryFilter* filter)
{
	...
	for (int j = 0; j < m_npolys; ++j)
	{
		...
		for (int k = 0; k < nsegs; ++k)
		{
			...
			float tseg;
			const float distSqr = dtDistancePtSegSqr2DTP(pos, s, s+3, tseg);
...
bool dtLocalBoundaryTP::isValid(dtNavMeshQueryTP* navquery, const dtQueryFilter* filter)
{
	if (!m_npolys)
		return false;
