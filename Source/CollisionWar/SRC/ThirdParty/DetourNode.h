...
{
public:
	dtNodePoolTP(int maxNodes, int hashSize);
	~dtNodePoolTP();
	void clear();
...
private:
	...
	dtNodePoolTP(const dtNodePoolTP&);
	dtNodePoolTP& operator=(const dtNodePoolTP&);
...
private:
	// Explicitly disabled copy constructor and copy assignment operator.
	dtNodeQueueTP(const dtNodeQueueTP&);
	dtNodeQueueTP& operator=(const dtNodeQueueTP&);
