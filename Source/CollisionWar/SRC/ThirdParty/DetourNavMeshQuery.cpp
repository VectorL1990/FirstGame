...
dtNavMeshQueryTP* dtAllocNavMeshQuery()
{
	void* mem = dtAlloc(sizeof(dtNavMeshQueryTP), DT_ALLOC_PERM);
	if (!mem) return 0;
	return new(mem) dtNavMeshQueryTP;
}

void dtFreeNavMeshQueryTP(dtNavMeshQueryTP* navmesh)
{
	if (!navmesh) return;
	navmesh->~dtNavMeshQueryTP();
	dtFreeTP(navmesh);
}

//////////////////////////////////////////////////////////////////////////////////////////

/// @class dtNavMeshQueryTP
///
...
///
/// @see dtNavMeshTP, dtQueryFilter, #dtAllocNavMeshQuery(), #dtAllocNavMeshQuery()

dtNavMeshQueryTP::dtNavMeshQueryTP() :
	m_nav(0),
	m_tinyNodePool(0),
	m_nodePool(0),
	m_openList(0)
...
{
	if (m_tinyNodePool)
		m_tinyNodePool->~dtNodePoolTP();
	if (m_nodePool)
		m_nodePool->~dtNodePoolTP();
	if (m_openList)
		m_openList->~dtNodeQueueTP();
	dtFreeTP(m_tinyNodePool);
	dtFreeTP(m_nodePool);
	dtFreeTP(m_openList);
}
...
dtStatus dtNavMeshQueryTP::init(const dtNavMeshTP* nav, const int maxNodes)
{
	...
	if (!m_nodePool || m_nodePool->getMaxNodes() < maxNodes)
	{
		...
		m_nodePool = new (dtAlloc(sizeof(dtNodePoolTP), DT_ALLOC_PERM)) dtNodePoolTP(maxNodes, dtNextPow2(maxNodes/4));
		if (!m_nodePool)
	...
	if (!m_tinyNodePool)
	{
		m_tinyNodePool = new (dtAlloc(sizeof(dtNodePoolTP), DT_ALLOC_PERM)) dtNodePoolTP(64, 32);
		if (!m_tinyNodePool)
			return DT_FAILURE | DT_OUT_OF_MEMORY;
	}
	else
	...
	if (!m_openList || m_openList->getCapacity() < maxNodes)
	{
		...
		m_openList = new (dtAlloc(sizeof(dtNodeQueueTP), DT_ALLOC_PERM)) dtNodeQueueTP(maxNodes);
		if (!m_openList)
			return DT_FAILURE | DT_OUT_OF_MEMORY;
	}
	else
...
dtStatus dtNavMeshQueryTP::findRandomPoint(const dtQueryFilter* filter, float (*frand)(),
										 dtPolyRef* randomRef, float* randomPt) const
{
	...
	float pt[3];
	dtRandomPointInConvexPolyTP(verts, poly->vertCount, areas, s, t, pt);
...
dtStatus dtNavMeshQueryTP::findRandomPointAroundCircle(dtPolyRef startRef, const float* centerPos, const float maxRadius,
													 ...
													 dtPolyRef* randomRef, float* randomPt) const
{
	...
	while (!m_openList->empty())
	{
		...
		for (unsigned int i = bestPoly->firstLink; i != DT_NULL_LINK; i = bestTile->links[i].next)
		{
			...
			float tseg;
			float distSqr = dtDistancePtSegSqr2DTP(centerPos, va, vb, tseg);
	...
	float pt[3];
	dtRandomPointInConvexPolyTP(verts, randomPoly->vertCount, areas, s, t, pt);
...
///
dtStatus dtNavMeshQueryTP::closestPointOnPoly(dtPolyRef ref, const float* pos, float* closest, bool* posOverPoly) const
{
	dtAssert(m_nav);
	const dtMeshTile* tile = 0;
	const dtPoly* poly = 0;
	...
	dtVcopy(closest, pos);
	if (!dtDistancePtPolyEdgesSqrTP(pos, verts, nv, edged, edget))
	{
	...
	for (int j = 0; j < pd->triCount; ++j)
	{
		...
		float h;
		if (dtClosestHeightPointTriangleTP(closest, v[0], v[1], v[2], h))
		{
			closest[1] = h;
			break;
		}
...
/// 
dtStatus dtNavMeshQueryTP::closestPointOnPolyBoundary(dtPolyRef ref, const float* pos, float* closest) const
{
	dtAssert(m_nav);
	...
	bool inside = dtDistancePtPolyEdgesSqrTP(pos, verts, nv, edged, edget);
	if (inside)
	{
...
/// of the polygon.
/// 
dtStatus dtNavMeshQueryTP::getPolyHeight(dtPolyRef ref, const float* pos, float* height) const
{
	dtAssert(m_nav);
	...
	else
	{
		...
		for (int j = 0; j < pd->triCount; ++j)
		{
			...
			float h;
			if (dtClosestHeightPointTriangleTP(pos, v[0], v[1], v[2], h))
			{
				if (height)
					*height = h;
				return DT_SUCCESS;
...
{
	const dtNavMeshQueryTP* m_query;
...
public:
	dtFindNearestPolyQuery(const dtNavMeshQueryTP* query, const float* center)
...
///
dtStatus dtNavMeshQueryTP::findNearestPoly(const float* center, const float* halfExtents,
...
void dtNavMeshQueryTP::queryPolygonsInTile(const dtMeshTile* tile, const float* qmin, const float* qmax,
										 const dtQueryFilter* filter, dtPolyQuery* query) const
{
	...
	else
	{
		...
		for (int i = 0; i < tile->header->polyCount; ++i)
		{
			...
			//GEngine->AddOnScreenDebugMessage(-1, 100.f, FColor::Black, TEXT("min bound x:") + FString::SanitizeFloat(bmin[0]) + TEXT(",") + TEXT("y: ") + FString::SanitizeFloat(bmin[1]) + TEXT(",") + TEXT("z: ") + FString::SanitizeFloat(bmin[2]));
			//GEngine->AddOnScreenDebugMessage(-1, 100.f, FColor::Black, TEXT("max bound x:") + FString::SanitizeFloat(bmax[0]) + TEXT(",") + TEXT("y: ") + FString::SanitizeFloat(bmax[1]) + TEXT(",") + TEXT("z: ") + FString::SanitizeFloat(bmax[2]));
...
///
dtStatus dtNavMeshQueryTP::queryPolygons(const float* center, const float* halfExtents,
									   const dtQueryFilter* filter,
...
///
dtStatus dtNavMeshQueryTP::findPath(dtPolyRef startRef, dtPolyRef endRef,
...
dtStatus dtNavMeshQueryTP::getPathToNode(dtNode* endNode, dtPolyRef* path, int* pathCount, int maxPath) const
{
...
/// path query.
///
dtStatus dtNavMeshQueryTP::initSlicedFindPath(dtPolyRef startRef, dtPolyRef endRef,
											const float* startPos, const float* endPos,
...
dtStatus dtNavMeshQueryTP::updateSlicedFindPath(const int maxIter, int* doneIters)
{
...
dtStatus dtNavMeshQueryTP::finalizeSlicedFindPath(dtPolyRef* path, int* pathCount, const int maxPath)
{
	*pathCount = 0;
...
dtStatus dtNavMeshQueryTP::finalizeSlicedFindPathPartial(const dtPolyRef* existing, const int existingSize,
...
dtStatus dtNavMeshQueryTP::appendVertex(const float* pos, const unsigned char flags, const dtPolyRef ref,
...
dtStatus dtNavMeshQueryTP::appendPortals(const int startIdx, const int endIdx, const float* endPos, const dtPolyRef* path,
									  float* straightPath, unsigned char* straightPathFlags, dtPolyRef* straightPathRefs,
									  int* straightPathCount, const int maxStraightPath, const int options) const
{
	...
	for (int i = startIdx; i < endIdx; i++)
	{
		...
		// Append intersection
		float s,t;
		if (dtIntersectSegSeg2DTP(startPos, endPos, left, right, s, t))
		{
			float pt[3];
...
/// position.
///
dtStatus dtNavMeshQueryTP::findStraightPath(const float* startPos, const float* endPos,
	...
	if (pathSize > 1)
	{
		...
		for (int i = 0; i < pathSize; ++i)
		{
			...
			if (i+1 < pathSize)
			{
				...
				{
					float t;
					if (dtDistancePtSegSqr2DTP(portalApex, left, right, t) < dtSqr(0.001f))
						continue;
				}
			}
			else
...
/// position.
///
dtStatus dtNavMeshQueryTP::moveAlongSurface(dtPolyRef startRef, const float* startPos, const float* endPos,
										  const dtQueryFilter* filter,
										  float* resultPos, dtPolyRef* visited, int* visitedCount, const int maxVisitedSize) const
{
	...
	*visitedCount = 0;
	...
	while (nstack)
	{
		...
		if (dtPointInPolygonTP(endPos, verts, nverts))
		{
			bestNode = curNode;
		...
		for (int i = 0, j = (int)curPoly->vertCount-1; i < (int)curPoly->vertCount; j = i++)
		{
			...
			if (!nneis)
			{
				...
				float tseg;
				const float distSqr = dtDistancePtSegSqr2DTP(endPos, vj, vi, tseg);
				if (distSqr < bestDist)
				{
			...
			else
			{
				for (int k = 0; k < nneis; ++k)
				{
					...
					const float* vi = &verts[i*3];
					float tseg;
					float distSqr = dtDistancePtSegSqr2DTP(searchPos, vj, vi, tseg);
...
dtStatus dtNavMeshQueryTP::getPortalPoints(dtPolyRef from, dtPolyRef to, float* left, float* right,
...
dtStatus dtNavMeshQueryTP::getPortalPoints(dtPolyRef from, const dtPoly* fromPoly, const dtMeshTile* fromTile,
...
dtStatus dtNavMeshQueryTP::getEdgeMidPoint(dtPolyRef from, dtPolyRef to, float* mid) const
{
	float left[3], right[3];
...
dtStatus dtNavMeshQueryTP::getEdgeMidPoint(dtPolyRef from, const dtPoly* fromPoly, const dtMeshTile* fromTile,
										 dtPolyRef to, const dtPoly* toPoly, const dtMeshTile* toTile,
										 float* mid) const
{
	float left[3], right[3];
...
///
dtStatus dtNavMeshQueryTP::raycast(dtPolyRef startRef, const float* startPos, const float* endPos,
								 const dtQueryFilter* filter,
								 ...
								 dtRaycastHit* hit, dtPolyRef prevRef) const
{
	...
	while (curRef)
	{
		...
		float tmin, tmax;
		int segMin, segMax;
		if (!dtIntersectSegmentPoly2DTP(startPos, endPos, verts, nv, tmin, tmax, segMin, segMax))
		{
...
/// filled to capacity.
/// 
dtStatus dtNavMeshQueryTP::findPolysAroundCircle(dtPolyRef startRef, const float* centerPos, const float radius,
											   const dtQueryFilter* filter,
...
///
dtStatus dtNavMeshQueryTP::findPolysAroundShape(dtPolyRef startRef, const float* verts, const int nverts,
											  const dtQueryFilter* filter,
											  dtPolyRef* resultRef, dtPolyRef* resultParent, float* resultCost,
											  int* resultCount, const int maxResult) const
{
	...
	*resultCount = 0;
	...
	while (!m_openList->empty())
	{
		...
		for (unsigned int i = bestPoly->firstLink; i != DT_NULL_LINK; i = bestTile->links[i].next)
		{
			...
			float tmin, tmax;
			int segMin, segMax;
			if (!dtIntersectSegmentPoly2DTP(va, vb, verts, nverts, tmin, tmax, segMin, segMax))
				continue;
...
dtStatus dtNavMeshQueryTP::getPathFromDijkstraSearch(dtPolyRef endRef, dtPolyRef* path, int* pathCount, int maxPath) const
{
...
/// be filled to capacity.
/// 
dtStatus dtNavMeshQueryTP::findLocalNeighbourhood(dtPolyRef startRef, const float* centerPos, const float radius,
												const dtQueryFilter* filter,
	...
	while (nstack)
	{
		...
		for (unsigned int i = curPoly->firstLink; i != DT_NULL_LINK; i = curTile->links[i].next)
		{
			...
			for (int j = 0; j < n; ++j)
			{
				...
				if (dtOverlapPolyPoly2DTP(pa,npa, pb,npb))
				{
					overlap = true;
					break;
				}
...
/// 
dtStatus dtNavMeshQueryTP::getPolyWallSegments(dtPolyRef ref, const dtQueryFilter* filter,
...
///
dtStatus dtNavMeshQueryTP::findDistanceToWall(dtPolyRef startRef, const float* centerPos, const float maxRadius,
											const dtQueryFilter* filter,
											float* hitDist, float* hitPos, float* hitNormal) const
{
	...
	while (!m_openList->empty())
	{
		...
		for (int i = 0, j = (int)bestPoly->vertCount-1; i < (int)bestPoly->vertCount; j = i++)
		{
			...
			float tseg;
			float distSqr = dtDistancePtSegSqr2DTP(centerPos, vj, vi, tseg);
...
bool dtNavMeshQueryTP::isValidPolyRef(dtPolyRef ref, const dtQueryFilter* filter) const
{
	const dtMeshTile* tile = 0;
	const dtPoly* poly = 0;
...
/// 
bool dtNavMeshQueryTP::isInClosedList(dtPolyRef ref) const
{
