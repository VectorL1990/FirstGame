...
void dtPathQueueTP::purge()
{
	dtFreeNavMeshQueryTP(m_navquery);
	m_navquery = 0;
	for (int i = 0; i < MAX_QUEUE; ++i)
	{
		dtFreeTP(m_queue[i].path);
		m_queue[i].path = 0;
	}
}

bool dtPathQueueTP::init(const int maxPathSize, const int maxSearchNodeCount, dtNavMeshTP* nav)
{
	purge();
...
void dtPathQueueTP::update(const int maxIters)
{
...
dtPathQueueRef dtPathQueueTP::request(dtPolyRef startRef, dtPolyRef endRef,
...
dtStatus dtPathQueueTP::getRequestStatus(dtPathQueueRef ref) const
{
	for (int i = 0; i < MAX_QUEUE; ++i)
	{
...
dtStatus dtPathQueueTP::getPathResult(dtPathQueueRef ref, dtPolyRef* path, int* pathSize, const int maxPath)
{
	for (int i = 0; i < MAX_QUEUE; ++i)
	{
		if (m_queue[i].ref == ref)
