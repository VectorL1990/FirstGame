...
dtTileCacheTP* dtAllocTileCache()
{
	void* mem = dtAlloc(sizeof(dtTileCacheTP), DT_ALLOC_PERM);
	if (!mem) return 0;
	return new(mem) dtTileCacheTP;
}

void dtFreeTileCacheTP(dtTileCacheTP* tc)
{
	if (!tc) return;
	tc->~dtTileCacheTP();
	dtFreeTP(tc);
...
struct NavMeshTileBuildContext
{
	...
	void purge()
	{
		dtFreeTileCacheLayerTP(alloc, layer);
		layer = 0;
		dtFreeTileCacheContourSetTP(alloc, lcset);
		lcset = 0;
		dtFreeTileCachePolyMeshTP(alloc, lmesh);
		lmesh = 0;
	}
...
const dtCompressedTile* dtTileCacheTP::getTileByRef(dtCompressedTileRef ref) const
{
	if (!ref)
		return 0;
...
dtStatus dtTileCacheTP::init(const dtTileCacheParams* params,
...
int dtTileCacheTP::getTilesAt(const int tx, const int ty, dtCompressedTileRef* tiles, const int maxTiles) const 
{
	int n = 0;
...
dtCompressedTile* dtTileCacheTP::getTileAt(const int tx, const int ty, const int tlayer)
{
	// Find tile based on hash.
	int h = computeTileHash(tx,ty,m_tileLutMask);
	dtCompressedTile* tile = m_posLookup[h];
...
dtCompressedTileRef dtTileCacheTP::getTileRef(const dtCompressedTile* tile) const
{
	if (!tile) return 0;
...
dtObstacleRef dtTileCacheTP::getObstacleRef(const dtTileCacheObstacle* ob) const
{
	if (!ob) return 0;
...
const dtTileCacheObstacle* dtTileCacheTP::getObstacleByRef(dtObstacleRef ref)
{
	if (!ref)
		return 0;
...
dtStatus dtTileCacheTP::addTile(unsigned char* data, const int dataSize, unsigned char flags, dtCompressedTileRef* result)
{
...
dtStatus dtTileCacheTP::removeTile(dtCompressedTileRef ref, unsigned char** data, int* dataSize)
{
	if (!ref)
		return DT_FAILURE | DT_INVALID_PARAM;
	unsigned int tileIndex = decodeTileIdTile(ref);
...
dtStatus dtTileCacheTP::addObstacle(const float* pos, const float radius, const float height, dtObstacleRef* result)
{
	

	/*ofstream file;
	file.open("D:\testRC.txt");
	file << "addObstacle";
	file.close();*/
...
dtStatus dtTileCacheTP::addBoxObstacle(const float* bmin, const float* bmax, dtObstacleRef* result)
{
	if (m_nreqs >= MAX_REQUESTS)
		return DT_FAILURE | DT_BUFFER_TOO_SMALL;
...
dtStatus dtTileCacheTP::addBoxObstacle(const float* center, const float* halfExtents, const float yRadians, dtObstacleRef* result)
{
	if (m_nreqs >= MAX_REQUESTS)
		return DT_FAILURE | DT_BUFFER_TOO_SMALL;
...
dtStatus dtTileCacheTP::removeObstacle(const dtObstacleRef ref)
{
	if (!ref)
		return DT_SUCCESS;
	if (m_nreqs >= MAX_REQUESTS)
...
dtStatus dtTileCacheTP::queryTiles(const float* bmin, const float* bmax,
...
dtStatus dtTileCacheTP::update(const float /*dt*/, dtNavMeshTP* navmesh,
							 bool* upToDate)
{
	if (m_nupdate == 0)
	{
...
dtStatus dtTileCacheTP::buildNavMeshTilesAt(const int tx, const int ty, dtNavMeshTP* navmesh)
{
	const int MAX_TILES = 32;
	dtCompressedTileRef tiles[MAX_TILES];
	const int ntiles = getTilesAt(tx,ty,tiles,MAX_TILES);
...
dtStatus dtTileCacheTP::buildNavMeshTile(const dtCompressedTileRef ref, dtNavMeshTP* navmesh)
{	
	dtAssert(m_talloc);
	dtAssert(m_tcomp);
	...
	status = dtDecompressTileCacheLayerTP(m_talloc, m_tcomp, tile->data, tile->dataSize, &bc.layer);
	if (dtStatusFailed(status))
		return status;
	...
	for (int i = 0; i < m_params.maxObstacles; ++i)
	{
		...
		if (contains(ob->touched, ob->ntouched, ref))
		{
			...
			{
				dtMarkCylinderAreaTP(*bc.layer, tile->header->bmin, m_params.cs, m_params.ch,
							    ob->cylinder.pos, ob->cylinder.radius, ob->cylinder.height, 0);
			}
			else if (ob->type == DT_OBSTACLE_BOX)
			{
				dtMarkBoxAreaTP(*bc.layer, tile->header->bmin, m_params.cs, m_params.ch,
	...
	bc.lcset = dtAllocTileCacheContourSetTP(m_talloc);
	if (!bc.lcset)
		return DT_FAILURE | DT_OUT_OF_MEMORY;
	...
	bc.lmesh = dtAllocTileCachePolyMeshTP(m_talloc);
	if (!bc.lmesh)
		return DT_FAILURE | DT_OUT_OF_MEMORY;
	...
	int navDataSize = 0;
	if (!dtCreateNavMeshDataTP(&params, &navData, &navDataSize))
		return DT_FAILURE;
...
void dtTileCacheTP::calcTightTileBounds(const dtTileCacheLayerHeader* header, float* bmin, float* bmax) const
{
...
void dtTileCacheTP::getObstacleBounds(const struct dtTileCacheObstacle* ob, float* bmin, float* bmax) const
{
	if (ob->type == DT_OBSTACLE_CYLINDER)
	{
