...
dtNavMeshTP* dtAllocNavMesh()
{
	void* mem = dtAlloc(sizeof(dtNavMeshTP), DT_ALLOC_PERM);
	if (!mem) return 0;
	return new(mem) dtNavMeshTP;
}
...
/// flag set.
void dtFreeNavMeshTP(dtNavMeshTP* navmesh)
{
	if (!navmesh) return;
	navmesh->~dtNavMeshTP();
	dtFreeTP(navmesh);
...
-# Create a tile data array using dtCreateNavMeshDataTP().
-# Allocate at dtNavMeshTP object and initialize it. (For single tile navigation meshes,
   the tile data is loaded during this step.)
-# For multi-tile navigation meshes, load the tile data using dtNavMeshTP::addTile().

Notes:

- This class is usually used in conjunction with the dtNavMeshQueryTP class for pathfinding.
- Technically, all navigation meshes are tiled. A 'solo' mesh is simply a navigation mesh initialized 
  to have only a single tile.
- This class does not implement any asynchronous methods. So the ::dtStatus result of all methods will 
  always contain either a success or failure flag.

@see dtNavMeshQueryTP, dtCreateNavMeshDataTP, dtNavMeshCreateParams, #dtAllocNavMesh, #dtFreeNavMeshTP
*/

dtNavMeshTP::dtNavMeshTP() :
	m_tileWidth(0),
	m_tileHeight(0),
	m_maxTiles(0),
	m_tileLutSize(0),
...
dtNavMeshTP::~dtNavMeshTP()
{
	for (int i = 0; i < m_maxTiles; ++i)
	{
		...
		{
			dtFreeTP(m_tiles[i].data);
			m_tiles[i].data = 0;
			m_tiles[i].dataSize = 0;
		}
	}
	dtFreeTP(m_posLookup);
	dtFreeTP(m_tiles);
}
		
dtStatus dtNavMeshTP::init(const dtNavMeshParams* params)
{
...
dtStatus dtNavMeshTP::init(unsigned char* data, const int dataSize, const int flags)
{
...
const dtNavMeshParams* dtNavMeshTP::getParams() const
{
	return &m_params;
}

//////////////////////////////////////////////////////////////////////////////////////////
int dtNavMeshTP::findConnectingPolys(const float* va, const float* vb,
...
void dtNavMeshTP::unconnectLinks(dtMeshTile* tile, dtMeshTile* target)
{
...
void dtNavMeshTP::connectExtLinks(dtMeshTile* tile, dtMeshTile* target, int side)
{
	if (!tile) return;
...
void dtNavMeshTP::connectExtOffMeshLinks(dtMeshTile* tile, dtMeshTile* target, int side)
{
	if (!tile) return;
...
void dtNavMeshTP::connectIntLinks(dtMeshTile* tile)
{
	if (!tile) return;
...
void dtNavMeshTP::baseOffMeshLinks(dtMeshTile* tile)
{
	if (!tile) return;
...
void dtNavMeshTP::closestPointOnPoly(dtPolyRef ref, const float* pos, float* closest, bool* posOverPoly) const
{
...
dtPolyRef dtNavMeshTP::findNearestPolyInTile(const dtMeshTile* tile,
...
int dtNavMeshTP::queryPolygonsInTile(const dtMeshTile* tile, const float* qmin, const float* qmax,
...
///
/// @see dtCreateNavMeshDataTP, #removeTile
dtStatus dtNavMeshTP::addTile(unsigned char* data, int dataSize, int flags,
...
const dtMeshTile* dtNavMeshTP::getTileAt(const int x, const int y, const int layer) const
{
...
int dtNavMeshTP::getNeighbourTilesAt(const int x, const int y, const int side, dtMeshTile** tiles, const int maxTiles) const
{
	int nx = x, ny = y;
	switch (side)
	{
...
int dtNavMeshTP::getTilesAt(const int x, const int y, dtMeshTile** tiles, const int maxTiles) const
{
	int n = 0;
...
int dtNavMeshTP::getTilesAt(const int x, const int y, dtMeshTile const** tiles, const int maxTiles) const
{
	int n = 0;
...
dtTileRef dtNavMeshTP::getTileRefAt(const int x, const int y, const int layer) const
{
	// Find tile based on hash.
	int h = computeTileHash(x,y,m_tileLutMask);
	dtMeshTile* tile = m_posLookup[h];
...
const dtMeshTile* dtNavMeshTP::getTileByRef(dtTileRef ref) const
{
	if (!ref)
		return 0;
...
int dtNavMeshTP::getMaxTiles() const
{
	return m_maxTiles;
}

dtMeshTile* dtNavMeshTP::getTile(int i)
{
	return &m_tiles[i];
}

const dtMeshTile* dtNavMeshTP::getTile(int i) const
{
	return &m_tiles[i];
}

void dtNavMeshTP::calcTileLoc(const float* pos, int* tx, int* ty) const
{
	*tx = (int)floorf((pos[0]-m_orig[0]) / m_tileWidth);
	*ty = (int)floorf((pos[2]-m_orig[2]) / m_tileHeight);
}

dtStatus dtNavMeshTP::getTileAndPolyByRef(const dtPolyRef ref, const dtMeshTile** tile, const dtPoly** poly) const
{
...
void dtNavMeshTP::getTileAndPolyByRefUnsafe(const dtPolyRef ref, const dtMeshTile** tile, const dtPoly** poly) const
{
	unsigned int salt, it, ip;
	decodePolyId(ref, salt, it, ip);
	*tile = &m_tiles[it];
...
bool dtNavMeshTP::isValidPolyRef(dtPolyRef ref) const
{
	if (!ref) return false;
	unsigned int salt, it, ip;
	decodePolyId(ref, salt, it, ip);
...
///
/// @see #addTile
dtStatus dtNavMeshTP::removeTile(dtTileRef ref, unsigned char** data, int* dataSize)
{
	if (!ref)
		return DT_FAILURE | DT_INVALID_PARAM;
	unsigned int tileIndex = decodePolyIdTile((dtPolyRef)ref);
...
dtTileRef dtNavMeshTP::getTileRef(const dtMeshTile* tile) const
{
	if (!tile) return 0;
...
/// }
/// @endcode
dtPolyRef dtNavMeshTP::getPolyRefBase(const dtMeshTile* tile) const
{
	if (!tile) return 0;
	const unsigned int it = (unsigned int)(tile - m_tiles);
...
///  @see #storeTileState
int dtNavMeshTP::getTileStateSize(const dtMeshTile* tile) const
{
	if (!tile) return 0;
...
dtStatus dtNavMeshTP::storeTileState(const dtMeshTile* tile, unsigned char* data, const int maxDataSize) const
{
...
/// @see #storeTileState
dtStatus dtNavMeshTP::restoreTileState(dtMeshTile* tile, const unsigned char* data, const int maxDataSize)
{
	// Make sure there is enough space to store the state.
	const int sizeReq = getTileStateSize(tile);
	if (maxDataSize < sizeReq)
...
dtStatus dtNavMeshTP::getOffMeshConnectionPolyEndPoints(dtPolyRef prevRef, dtPolyRef polyRef, float* startPos, float* endPos) const
{
	unsigned int salt, it, ip;
...
const dtOffMeshConnection* dtNavMeshTP::getOffMeshConnectionByRef(dtPolyRef ref) const
{
	unsigned int salt, it, ip;
...
dtStatus dtNavMeshTP::setPolyFlags(dtPolyRef ref, unsigned short flags)
{
	if (!ref) return DT_FAILURE;
	unsigned int salt, it, ip;
	decodePolyId(ref, salt, it, ip);
...
dtStatus dtNavMeshTP::getPolyFlags(dtPolyRef ref, unsigned short* resultFlags) const
{
	if (!ref) return DT_FAILURE;
	unsigned int salt, it, ip;
	decodePolyId(ref, salt, it, ip);
...
dtStatus dtNavMeshTP::setPolyArea(dtPolyRef ref, unsigned char area)
{
	if (!ref) return DT_FAILURE;
	unsigned int salt, it, ip;
	decodePolyId(ref, salt, it, ip);
...
dtStatus dtNavMeshTP::getPolyArea(dtPolyRef ref, unsigned char* resultArea) const
{
	if (!ref) return DT_FAILURE;
	unsigned int salt, it, ip;
	decodePolyId(ref, salt, it, ip);
