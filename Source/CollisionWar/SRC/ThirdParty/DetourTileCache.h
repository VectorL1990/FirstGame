...
public:
	...
	dtStatus update(const float dt, class dtNavMeshTP* navmesh, bool* upToDate = 0);
	
	dtStatus buildNavMeshTilesAt(const int tx, const int ty, class dtNavMeshTP* navmesh);
	
	dtStatus buildNavMeshTile(const dtCompressedTileRef ref, class dtNavMeshTP* navmesh);
...
private:
	...
	dtTileCacheTP(const dtTileCacheTP&);
	dtTileCacheTP& operator=(const dtTileCacheTP&);
	...
	static const int MAX_REQUESTS = 7200;
	ObstacleRequest m_reqs[MAX_REQUESTS];
	int m_nreqs;
	
	static const int MAX_UPDATE = 7200;
