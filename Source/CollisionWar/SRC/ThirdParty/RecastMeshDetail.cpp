...
struct rcHeightPatch
{
	...
	inline ~rcHeightPatch() { rcFreeTP(data); }
	unsigned short* data;
...
static int addEdge(rcContextTP* ctx, int* edges, int& nedges, const int maxEdges, int s, int t, int l, int r)
{
	if (nedges >= maxEdges)
	{
...
static void completeFacet(rcContextTP* ctx, const float* pts, int npts, int* edges, int& nedges, const int maxEdges, int& nfaces, int e)
{
...
static void delaunayHull(rcContextTP* ctx, const int npts, const float* pts,
...
static bool buildPolyDetail(rcContextTP* ctx, const float* in, const int nin,
							...
							rcIntArray& tris, rcIntArray& edges, rcIntArray& samples)
{
	...
	if (ntris > MAX_TRIS)
	{
		...
		ctx->log(RC_LOG_ERROR, "rcBuildPolyMeshDetailTP: Shrinking triangle count from %d to max %d.", ntris, MAX_TRIS);
	}
	
	return true;
}

static void seedArrayWithPolyCenter(rcContextTP* ctx, const rcCompactHeightfield& chf,
...
static void getHeightData(rcContextTP* ctx, const rcCompactHeightfield& chf,
						  const unsigned short* poly, const int npoly,
						  const unsigned short* verts, const int bs,
...
///
/// @see rcAllocPolyMeshDetailTP, rcPolyMesh, rcCompactHeightfield, rcPolyMeshDetail, rcConfig
						   ...
						   rcPolyMeshDetail& dmesh)
{
	...
	if (!bounds)
	{
		ctx->log(RC_LOG_ERROR, "rcBuildPolyMeshDetailTP: Out of memory 'bounds' (%d).", mesh.npolys*4);
		return false;
	}
	rcScopedDelete<float> poly((float*)rcAlloc(sizeof(float)*nvp*3, RC_ALLOC_TEMP));
	if (!poly)
	{
		ctx->log(RC_LOG_ERROR, "rcBuildPolyMeshDetailTP: Out of memory 'poly' (%d).", nvp*3);
		return false;
	}
	...
	if (!hp.data)
	{
		ctx->log(RC_LOG_ERROR, "rcBuildPolyMeshDetailTP: Out of memory 'hp.data' (%d).", maxhw*maxhh);
		return false;
	}
	...
	if (!dmesh.meshes)
	{
		ctx->log(RC_LOG_ERROR, "rcBuildPolyMeshDetailTP: Out of memory 'dmesh.meshes' (%d).", dmesh.nmeshes*4);
		return false;
	}
	...
	if (!dmesh.verts)
	{
		ctx->log(RC_LOG_ERROR, "rcBuildPolyMeshDetailTP: Out of memory 'dmesh.verts' (%d).", vcap*3);
		return false;
	}
	dmesh.ntris = 0;
	...
	if (!dmesh.tris)
	{
		ctx->log(RC_LOG_ERROR, "rcBuildPolyMeshDetailTP: Out of memory 'dmesh.tris' (%d).", tcap*4);
		return false;
	}
	...
	for (int i = 0; i < mesh.npolys; ++i)
	{
		...
		if (dmesh.nverts+nverts > vcap)
		{
			...
			if (!newv)
			{
				ctx->log(RC_LOG_ERROR, "rcBuildPolyMeshDetailTP: Out of memory 'newv' (%d).", vcap*3);
				return false;
			}
			if (dmesh.nverts)
		...
		if (dmesh.ntris+ntris > tcap)
		{
			...
			if (!newt)
			{
				ctx->log(RC_LOG_ERROR, "rcBuildPolyMeshDetailTP: Out of memory 'newt' (%d).", tcap*4);
				return false;
			}
			if (dmesh.ntris)
...
/// @see rcAllocPolyMeshDetailTP, rcPolyMeshDetail
bool rcMergePolyMeshDetailsTP(rcContextTP* ctx, rcPolyMeshDetail** meshes, const int nmeshes, rcPolyMeshDetail& mesh)
{
	...
	if (!mesh.meshes)
	{
		ctx->log(RC_LOG_ERROR, "rcBuildPolyMeshDetailTP: Out of memory 'pmdtl.meshes' (%d).", maxMeshes*4);
		return false;
	}
	...
	if (!mesh.tris)
	{
		ctx->log(RC_LOG_ERROR, "rcBuildPolyMeshDetailTP: Out of memory 'dmesh.tris' (%d).", maxTris*4);
		return false;
	}
	...
	if (!mesh.verts)
	{
		ctx->log(RC_LOG_ERROR, "rcBuildPolyMeshDetailTP: Out of memory 'dmesh.verts' (%d).", maxVerts*3);
		return false;
	}
