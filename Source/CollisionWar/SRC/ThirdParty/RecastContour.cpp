...
static void mergeRegionHoles(rcContextTP* ctx, rcContourRegion& region)
{
...
///
/// @see rcAllocContourSetTP, rcCompactHeightfield, rcContourSet, rcConfig
					 ...
					 rcContourSet& cset, const int buildFlags)
{
	...
	if (!flags)
	{
		ctx->log(RC_LOG_ERROR, "rcBuildContoursTP: Out of memory 'flags' (%d).", chf.spanCount);
		return false;
	}
			...
			for (int i = (int)c.index, ni = (int)(c.index+c.count); i < ni; ++i)
			{
				...
				if (simplified.size()/4 >= 3)
				{
					if (cset.nconts >= maxContours)
					{
						...
						ctx->log(RC_LOG_WARNING, "rcBuildContoursTP: Expanding max contours from %d to %d.", oldMax, maxContours);
					}
					...
					if (!cont->verts)
					{
						ctx->log(RC_LOG_ERROR, "rcBuildContoursTP: Out of memory 'verts' (%d).", cont->nverts);
						return false;
					}
					...
					if (!cont->rverts)
					{
						ctx->log(RC_LOG_ERROR, "rcBuildContoursTP: Out of memory 'rverts' (%d).", cont->nrverts);
						return false;
					}
	...
	if (cset.nconts > 0)
	{
		...
		if (!winding)
		{
			ctx->log(RC_LOG_ERROR, "rcBuildContoursTP: Out of memory 'hole' (%d).", cset.nconts);
			return false;
		}
		int nholes = 0;
		...
		if (nholes > 0)
		{
			...
			if (!regions)
			{
				ctx->log(RC_LOG_ERROR, "rcBuildContoursTP: Out of memory 'regions' (%d).", nregions);
				return false;
			}
			...
			if (!holes)
			{
				ctx->log(RC_LOG_ERROR, "rcBuildContoursTP: Out of memory 'holes' (%d).", cset.nconts);
				return false;
			}
			...
			for (int i = 0; i < cset.nconts; ++i)
			{
				...
				if (winding[i] > 0)
				{
					if (regions[cont.reg].outline)
						ctx->log(RC_LOG_ERROR, "rcBuildContoursTP: Multiple outlines for region %d.", cont.reg);
			...
			for (int i = 0; i < nregions; i++)
			{
				...
				else
				{
					...
					ctx->log(RC_LOG_ERROR, "rcBuildContoursTP: Bad outline for region %d, contour simplification is likely too aggressive.", i);
				}
			}
		}
