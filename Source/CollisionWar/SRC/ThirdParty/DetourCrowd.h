...
/// crowd manager.
/// @ingroup crowd
/// @see dtObstacleAvoidanceParamsTP, dtCrowdTP::setObstacleAvoidanceParams(), dtCrowdTP::getObstacleAvoidanceParams(),
///		 dtCrowdAgentParams::obstacleAvoidanceType
static const int DT_CROWD_MAX_OBSTAVOIDANCE_PARAMS = 8;

/// The maximum number of query filter types supported by the crowd manager.
/// @ingroup crowd
/// @see dtQueryFilter, dtCrowdTP::getFilter() dtCrowdTP::getEditableFilter(),
///		dtCrowdAgentParams::queryFilterType
static const int DT_CROWD_MAX_QUERY_FILTER_TYPE = 16;

/// Provides neighbor data for agents managed by the crowd.
/// @ingroup crowd
/// @see dtCrowdAgent::neis, dtCrowdTP
...
/// Represents an agent managed by a #dtCrowdTP object.
/// @ingroup crowd
struct dtCrowdAgent
{
	int m_agentMark = -1;
	bool m_isPlayerAgent = false;
	...
	dtPathCorridorTP corridor;

	/// The local boundary data for the agent.
	dtLocalBoundaryTP boundary;
...
enum UpdateFlags
{
	...
	DT_CROWD_OPTIMIZE_VIS = 8,			///< Use #dtPathCorridorTP::optimizePathVisibility() to optimize the agent path.
	DT_CROWD_OPTIMIZE_TOPO = 16,		///< Use dtPathCorridorTP::optimizePathTopology() to optimize the agent path.
};
...
struct dtCrowdAgentDebugInfo
{
	...
	dtObstacleAvoidanceDebugDataTP* vod;
};
...
class dtCrowdTP
{
	...
	dtPathQueueTP m_pathq;

	dtObstacleAvoidanceParamsTP m_obstacleQueryParams[DT_CROWD_MAX_OBSTAVOIDANCE_PARAMS];
	dtObstacleAvoidanceQueryTP* m_obstacleQuery;
	
	dtProximityGridTP* m_grid;
...
public:
	...
	bool init(const int maxAgents, const float maxAgentRadius, dtNavMeshTP* nav, float xCoe, float yCoe, float zCoe);
	
	/// Sets the shared avoidance configuration for the specified index.
	///  @param[in]		idx		The index. [Limits: 0 <= value < #DT_CROWD_MAX_OBSTAVOIDANCE_PARAMS]
	///  @param[in]		params	The new configuration.
	void setObstacleAvoidanceParams(const int idx, const dtObstacleAvoidanceParamsTP* params);

	/// Gets the shared avoidance configuration for the specified index.
	///  @param[in]		idx		The index of the configuration to retreive. 
	///							[Limits:  0 <= value < #DT_CROWD_MAX_OBSTAVOIDANCE_PARAMS]
	/// @return The requested configuration.
	const dtObstacleAvoidanceParamsTP* getObstacleAvoidanceParams(const int idx) const;
	...
	int addAgent(const float* pos, const dtCrowdAgentParams* params, int agentMark, bool isPlayerAgent);
	...
	const dtProximityGridTP* getGrid() const { return m_grid; }

	/// Gets the crowd's path request queue.
	/// @return The crowd's path request queue.
	const dtPathQueueTP* getPathQueue() const { return &m_pathq; }

	/// Gets the query object used by the crowd.
	const dtNavMeshQueryTP* getNavMeshQuery() const { return m_navquery; }

private:
	// Explicitly disabled copy constructor and copy assignment operator.
	dtCrowdTP(const dtCrowdTP&);
	dtCrowdTP& operator=(const dtCrowdTP&);
};
...
Main class: #dtCrowdTP

The #dtNavMeshQueryTP and #dtPathCorridorTP classes provide perfectly good, easy 
to use path planning features. But in the end they only give you points that 
your navigation client should be moving toward. When it comes to deciding things 
like agent velocity and steering to avoid other agents, that is up to you to 
implement. Unless, of course, you decide to use #dtCrowdTP.
...
@struct dtCrowdAgentParams
@see dtCrowdAgent, dtCrowdTP::addAgent(), dtCrowdTP::updateAgentParameters()

@var dtCrowdAgentParams::obstacleAvoidanceType
@par

#dtCrowdTP permits agents to use different avoidance configurations.  This value 
is the index of the #dtObstacleAvoidanceParamsTP within the crowd.

@see dtObstacleAvoidanceParamsTP, dtCrowdTP::setObstacleAvoidanceParams(), 
	 dtCrowdTP::getObstacleAvoidanceParams()
...
@see dtPathCorridorTP::optimizePathVisibility()
