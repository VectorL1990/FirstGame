...
static bool mergeAndFilterRegions(rcContextTP* ctx, int minRegionArea, int mergeRegionSize,
...
static bool mergeAndFilterLayerRegions(rcContextTP* ctx, int minRegionArea,
									   unsigned short& maxRegionId,
									   rcCompactHeightfield& chf,
									   unsigned short* srcReg, rcIntArray& /*overlaps*/)
{
...
/// using #rcBuildRegionsTP or #rcBuildRegionsMonotoneTP.
/// 
/// After this step, the distance data is available via the rcCompactHeightfield::maxDistance
/// and rcCompactHeightfield::dist fields.
///
/// @see rcCompactHeightfield, rcBuildRegionsTP, rcBuildRegionsMonotoneTP
bool rcBuildDistanceFieldTP(rcContextTP* ctx, rcCompactHeightfield& chf)
{
	...
	if (!src)
	{
		ctx->log(RC_LOG_ERROR, "rcBuildDistanceFieldTP: Out of memory 'src' (%d).", chf.spanCount);
		return false;
	}
	unsigned short* dst = (unsigned short*)rcAlloc(sizeof(unsigned short)*chf.spanCount, RC_ALLOC_TEMP);
	if (!dst)
	{
		ctx->log(RC_LOG_ERROR, "rcBuildDistanceFieldTP: Out of memory 'dst' (%d).", chf.spanCount);
		rcFreeTP(src);
		return false;
	}
...
/// 
/// @warning The distance field must be created using #rcBuildDistanceFieldTP before attempting to build regions.
/// 
/// @see rcCompactHeightfield, rcCompactSpan, rcBuildDistanceFieldTP, rcBuildRegionsMonotoneTP, rcConfig
							...
							const int borderSize, const int minRegionArea, const int mergeRegionArea)
{
	...
	if (!srcReg)
	{
		ctx->log(RC_LOG_ERROR, "rcBuildRegionsMonotoneTP: Out of memory 'src' (%d).", chf.spanCount);
		return false;
	}
	...
	if (!sweeps)
	{
		ctx->log(RC_LOG_ERROR, "rcBuildRegionsMonotoneTP: Out of memory 'sweeps' (%d).", nsweeps);
		return false;
	}
					...
					const int borderSize, const int minRegionArea, const int mergeRegionArea)
{
	...
	if (!buf)
	{
		ctx->log(RC_LOG_ERROR, "rcBuildRegionsTP: Out of memory 'tmp' (%d).", chf.spanCount*4);
		return false;
	}
...
//		ctx->stopTimer(RC_TIMER_DIVIDE_TO_LEVELS);
			...
			for (int j = 0; j<lvlStacks[sId].size(); j += 3)
			{
				...
				if (i >= 0 && srcReg[i] == 0)
				{
					if (floodRegion(x, y, i, level, regionId, chf, srcReg, srcDist, stack))
					{
						if (regionId == 0xFFFF)
						{
							ctx->log(RC_LOG_ERROR, "rcBuildRegionsTP: Region ID overflow");
							return false;
						}
	...
	
	{
		...
		if (overlaps.size() > 0)
		{
			ctx->log(RC_LOG_ERROR, "rcBuildRegionsTP: %d overlapping regions.", overlaps.size());
		}
	}
