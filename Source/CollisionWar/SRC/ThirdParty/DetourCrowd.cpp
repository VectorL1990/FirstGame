...
dtCrowdTP* dtAllocCrowd()
{
	void* mem = dtAlloc(sizeof(dtCrowdTP), DT_ALLOC_PERM);
	if (!mem) return 0;
	return new(mem) dtCrowdTP;
}

void dtFreeCrowdTP(dtCrowdTP* ptr)
{
	if (!ptr) return;
	ptr->~dtCrowdTP();
	dtFreeTP(ptr);
...
static void integrate(dtCrowdAgent* ag, const float dt)
{
	...
	/*GEngine->AddOnScreenDebugMessage(-1, 10.f, FColor::Green, FString::SanitizeFloat(ag->m_agentMark) + TEXT("  Recast dvel is") + FString::SanitizeFloat(ag->vel[0]) +
		TEXT("  and  ") + FString::SanitizeFloat(ag->vel[1]) + TEXT("  and  ") + FString::SanitizeFloat(ag->vel[2]));*/
	...
	/*if (dtVlen(ag->vel) > 0.0001f)
		...
		dtVset(ag->vel,0,0,0);*/
	if (dtVlen(ag->vel) <= 0.0001f) dtVset(ag->vel, 0, 0, 0);
}
...
static int getNeighbours(const float* pos, const float height, const float range,
						 ...
						 dtCrowdAgent** agents, const int /*nagents*/, dtProximityGridTP* grid)
{
	int n = 0;
...
@see dtAllocCrowd(), dtFreeCrowdTP(), init(), dtCrowdAgent
...
void dtCrowdTP::purge()
{
	...
	dtFreeTP(m_activeAgents);
	m_activeAgents = 0;

	dtFreeTP(m_agentAnims);
	m_agentAnims = 0;
	
	dtFreeTP(m_pathResult);
	m_pathResult = 0;
	
	dtFreeProximityGridTP(m_grid);
	m_grid = 0;

	dtFreeObstacleAvoidanceQueryTP(m_obstacleQuery);
	m_obstacleQuery = 0;
	
	dtFreeNavMeshQueryTP(m_navquery);
	m_navquery = 0;
}
...
bool dtCrowdTP::init(const int maxAgents, const float maxAgentRadius, dtNavMeshTP* nav, float xCoe, float yCoe, float zCoe)
{
	purge();
	...
	dtVset(m_agentPlacementHalfExtents, m_maxAgentRadius*xCoe, m_maxAgentRadius*yCoe, m_maxAgentRadius*zCoe);
	...
	{
		dtObstacleAvoidanceParamsTP* params = &m_obstacleQueryParams[i];
		params->velBias = 0.4f;
...
void dtCrowdTP::setObstacleAvoidanceParams(const int idx, const dtObstacleAvoidanceParamsTP* params)
{
	if (idx >= 0 && idx < DT_CROWD_MAX_OBSTAVOIDANCE_PARAMS)
		memcpy(&m_obstacleQueryParams[idx], params, sizeof(dtObstacleAvoidanceParamsTP));
}

const dtObstacleAvoidanceParamsTP* dtCrowdTP::getObstacleAvoidanceParams(const int idx) const
{
	if (idx >= 0 && idx < DT_CROWD_MAX_OBSTAVOIDANCE_PARAMS)
...
int dtCrowdTP::getAgentCount() const
{
	return m_maxAgents;
}
...
const dtCrowdAgent* dtCrowdTP::getAgent(const int idx)
{
...
/// 
/// Agents in the pool may not be in use.  Check #dtCrowdAgent.active before using the returned object.
dtCrowdAgent* dtCrowdTP::getEditableAgent(const int idx)
{
	if (idx < 0 || idx >= m_maxAgents)
		return 0;
	return &m_agents[idx];
...
void dtCrowdTP::updateAgentParameters(const int idx, const dtCrowdAgentParams* params)
{
	if (idx < 0 || idx >= m_maxAgents)
		return;
...
int dtCrowdTP::addAgent(const float* pos, const dtCrowdAgentParams* params, int agentMark, bool isPlayerAgent)
{
	// Find empty slot.
	int idx = -1;
	for (int i = 0; i < m_maxAgents; ++i)
	...
	ag->m_agentMark = agentMark;
	ag->m_isPlayerAgent = isPlayerAgent;
...
void dtCrowdTP::removeAgent(const int idx)
{
	if (idx >= 0 && idx < m_maxAgents)
	{
...
bool dtCrowdTP::requestMoveTargetReplan(const int idx, dtPolyRef ref, const float* pos)
{
	if (idx < 0 || idx >= m_maxAgents)
		return false;
...
bool dtCrowdTP::requestMoveTarget(const int idx, dtPolyRef ref, const float* pos)
{
	if (idx < 0 || idx >= m_maxAgents)
		return false;
	if (!ref)
...
bool dtCrowdTP::requestMoveVelocity(const int idx, const float* vel)
{
	if (idx < 0 || idx >= m_maxAgents)
		return false;
...
bool dtCrowdTP::resetMoveTarget(const int idx)
{
	if (idx < 0 || idx >= m_maxAgents)
		return false;
...
int dtCrowdTP::getActiveAgents(dtCrowdAgent** agents, const int maxAgents)
{
	int n = 0;
	for (int i = 0; i < m_maxAgents; ++i)
	{
...
void dtCrowdTP::updateMoveRequest(const float /*dt*/)
{
	...
	for (int i = 0; i < m_maxAgents; ++i)
	{
		...
		if (ag->state == DT_CROWDAGENT_STATE_INVALID)
			continue;
		if (ag->targetState == DT_CROWDAGENT_TARGET_NONE || ag->targetState == DT_CROWDAGENT_TARGET_VELOCITY || ag->m_isPlayerAgent)
			continue;
...
void dtCrowdTP::updateTopologyOptimization(dtCrowdAgent** agents, const int nagents, const float dt)
{
	if (!nagents)
		return;
...
void dtCrowdTP::checkPathValidity(dtCrowdAgent** agents, const int nagents, const float dt)
{
...
void dtCrowdTP::update(const float dt, dtCrowdAgentDebugInfo* debug)
{
	m_velocitySampleCount = 0;
	...
	for (int i = 0; i < nagents; ++i)
	{
		...
		else
		{
			...
			/*GEngine->AddOnScreenDebugMessage(-1, 10.f, FColor::Cyan, FString::SanitizeFloat(ag->m_agentMark) + TEXT("  Recast dvel is") + FString::SanitizeFloat(dvel[0]) +
				TEXT("  and  ") + FString::SanitizeFloat(dvel[1]) + TEXT("  and  ") + FString::SanitizeFloat(dvel[2]));*/
			ag->desiredSpeed = ag->params.maxSpeed;
			dtVscale(dvel, dvel, ag->desiredSpeed * speedScale);
			/*GEngine->AddOnScreenDebugMessage(-1, 10.f, FColor::Red,FString::SanitizeFloat(ag->m_agentMark) + TEXT("  Recast dvel is") + FString::SanitizeFloat(dvel[0]) +
				TEXT("  and  ") + FString::SanitizeFloat(dvel[1]) + TEXT("  and  ") + FString::SanitizeFloat(dvel[2]));*/
		}
	...
	for (int i = 0; i < nagents; ++i)
	{
		...
		if (!ag->m_isPlayerAgent && (ag->params.updateFlags & DT_CROWD_OBSTACLE_AVOIDANCE))
		{
			...
			dtObstacleAvoidanceDebugDataTP* vod = 0;
			if (debugIdx == i) 
				vod = debug->vod;
			...
			const dtObstacleAvoidanceParamsTP* params = &m_obstacleQueryParams[ag->params.obstacleAvoidanceType];
		...
		else
		{
			...
			if (ag->m_isPlayerAgent) continue;
			else dtVcopy(ag->nvel, ag->dvel);
		}
	}
