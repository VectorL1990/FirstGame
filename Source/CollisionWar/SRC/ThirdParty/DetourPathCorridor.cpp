...
/**
@class dtPathCorridorTP
@par

The corridor is loaded with a path, usually obtained from a #dtNavMeshQueryTP::findPath() query. The corridor
...
-# Obtain a path from a #dtNavMeshQueryTP object.
...
All local mesh queries have distance limitations. (Review the #dtNavMeshQueryTP methods for details.) So the most accurate 
...
dtPathCorridorTP::dtPathCorridorTP() :
	m_path(0),
	m_npath(0),
	m_maxPath(0)
{
...
bool dtPathCorridorTP::init(const int maxPath)
{
	dtAssert(!m_path);
...
void dtPathCorridorTP::reset(dtPolyRef ref, const float* pos)
{
	dtAssert(m_path);
	dtVcopy(m_pos, pos);
...
detected in order to plan movement. It performs essentially the same function as #dtNavMeshQueryTP::findStraightPath.
...
*/
int dtPathCorridorTP::findCorners(float* cornerVerts, unsigned char* cornerFlags,
							  dtPolyRef* cornerPolys, const int maxCorners,
							  dtNavMeshQueryTP* navquery, const dtQueryFilter* /*filter*/)
{
	dtAssert(m_path);
	dtAssert(m_npath);
...
*/
void dtPathCorridorTP::optimizePathVisibility(const float* next, const float pathOptimizationRange,
										  dtNavMeshQueryTP* navquery, const dtQueryFilter* filter)
{
	dtAssert(m_path);
...
*/
bool dtPathCorridorTP::optimizePathTopology(dtNavMeshQueryTP* navquery, const dtQueryFilter* filter)
{
	dtAssert(navquery);
	dtAssert(filter);
	dtAssert(m_path);
...
bool dtPathCorridorTP::moveOverOffmeshConnection(dtPolyRef offMeshConRef, dtPolyRef* refs,
											   float* startPos, float* endPos,
											   dtNavMeshQueryTP* navquery)
{
	dtAssert(navquery);
	dtAssert(m_path);
	dtAssert(m_npath);
	...
	const dtNavMeshTP* nav = navquery->getAttachedNavMesh();
	dtAssert(nav);
...
*/
bool dtPathCorridorTP::movePosition(const float* npos, dtNavMeshQueryTP* navquery, const dtQueryFilter* filter)
{
	dtAssert(m_path);
	dtAssert(m_npath);
...
*/
bool dtPathCorridorTP::moveTargetPosition(const float* npos, dtNavMeshQueryTP* navquery, const dtQueryFilter* filter)
{
	dtAssert(m_path);
	dtAssert(m_npath);
...
void dtPathCorridorTP::setCorridor(const float* target, const dtPolyRef* path, const int npath)
{
	dtAssert(m_path);
	dtAssert(npath > 0);
...
bool dtPathCorridorTP::fixPathStart(dtPolyRef safeRef, const float* safePos)
{
	dtAssert(m_path);
...
bool dtPathCorridorTP::trimInvalidPath(dtPolyRef safeRef, const float* safePos,
									 dtNavMeshQueryTP* navquery, const dtQueryFilter* filter)
{
	dtAssert(navquery);
	dtAssert(filter);
...
bool dtPathCorridorTP::isValid(const int maxLookAhead, dtNavMeshQueryTP* navquery, const dtQueryFilter* filter)
{
